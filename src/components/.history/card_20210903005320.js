...
const Card = (article) => {
  ...
  //     <div class="img-container">
  const div_imgContainer = helper_create_tag({type:"div", classArray:["img-container"]})
  //       <img src={ authorPhoto }>
  const img = helper_create_tag({type:"img", textContent:article.authorPhoto})
    //     <span>By { authorName }</span>
  const span_author = helper_create_tag({type:"span", textContent:`By ${article.}`})
  //   </div>
  // </div>
}
